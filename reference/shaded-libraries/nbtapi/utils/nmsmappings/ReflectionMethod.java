...
public enum ReflectionMethod {
    ...
    private ReflectionMethod(ClassWrapper targetClass, Class<?>[] args, MinecraftVersion addedSince, MinecraftVersion removedAfter, Since ... methodnames) {
        ...
        boolean bl = specialCase = MinecraftVersion.isForgePresent() && this.name().equals("COMPOUND_MERGE") && MinecraftVersion.getVersion() == MinecraftVersion.MC1_7_R4;
        ...
        for (Since s : methodnames) {
            if (s.version.getVersionId() > server.getVersionId() || target.version.getVersionId() >= s.version.getVersionId()) continue;
            target = s;
        }
        ...
        try {
            ...
            catch (NoSuchMethodException | NullPointerException | SecurityException ex2) {
                MinecraftVersion.getLogger().warning("[NBTAPI] Unable to find the method '" + targetMethodName + "' in '" + (targetClass.getClazz() == null ? targetClass.getMojangName() : targetClass.getClazz().getSimpleName()) + "' Args: " + Arrays.toString(args) + " Enum: " + (Object)((Object)this));
            }
        }
    }
    ...
    public Object run(Object target, Object ... args) {
        ...
        try {
            ...
            throw new NbtApiException("Error while calling the method '" + this.methodName + "', loaded: " + this.loaded + ", Enum: " + (Object)((Object)this) + ", Passed Class: " + (target == null ? "null" : target.getClass()) + " Args: " + (args == null ? "null" : Arrays.toString(args)) + " Classes: " + (args == null ? "null" : Arrays.asList(args).stream().map(a -> a == null ? "NULL" : a.getClass().getName()).collect(Collectors.toList())), ex);
        }
    }
